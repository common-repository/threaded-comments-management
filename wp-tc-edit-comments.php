<?php
/**
 * Threaded Comments - Edit Comments Administration Screen.
 *
 * @package WordPress
...
require_once('includes/class-wp-tc-comments-list-table.php');

if ( !current_user_can('edit_posts') )
	wp_die(__('Cheatin&#8217; uh?'));

$post_id = isset( $_REQUEST['p'] ) ? absint( $_REQUEST['p'] ) : 0; //It's usually done in List_Table constructor
$threaded_view = isset( $_REQUEST['tv'] ) ? absint( $_REQUEST['tv'] ) : 1;
if(isset($_REQUEST['comment_status']) && $_REQUEST['comment_status'] != 'all') $threaded_view = 0;


if($post_id && $threaded_view){
	$wp_list_table = new WP_TC_Comments_List_Table;
	$inspect_thread = isset( $_REQUEST['t'] ) ? absint( $_REQUEST['t'] ) : -1;
} else {
    $wp_list_table = new WP_TC_NT_Comments_List_Table;
}
...
add_screen_option( 'expand', array('label' => _x( 'Comments', 'comments per page (screen options)' )) );//?
...
<?php //Add new submenu - Threaded view | Plain view?>

<?php unset($_REQUEST['tv']); ?>

<ul class='subsubsub' style="clear:both;width:100%;">
	<?php if((!isset($_REQUEST['comment_status']) || $_REQUEST['comment_status'] == "all") && isset($_REQUEST['p'])) {?> <li><a href="<?php echo esc_url( admin_url( 'edit-comments?'.http_build_query($_REQUEST))) ?>" <?php if($threaded_view == 1) echo "class='current'" ?>><?php _e("Threaded view", "wptc"); ?></a> |</li><?php }?>
	<li><a href="<?php echo esc_url( admin_url( 'edit-comments?tv=0&'.http_build_query($_REQUEST))) ?>" <?php if($threaded_view == 0 || !isset($_REQUEST['p'])) echo "class='current'" ?>> <?php _e("Plain view", "wptc"); ?></a></li>
</ul>
